...
struct GetDropGlueTypeFolder<'a, 'tcx: 'a> {
    tcx: &'a TyCtxt<'tcx>,
}

impl<'a, 'tcx> TypeFolder<'tcx> for GetDropGlueTypeFolder<'a, 'tcx> {
    ...
    fn fold_ty(&mut self, t: Ty<'tcx>) -> Ty<'tcx> {
        match t.sty {
            ty::TyTrait(ref trait_ty) => {

                let trait_ty = ty::TraitTy {
                    principal: trait_ty.principal.fold_with(self),
                    bounds: ty::ExistentialBounds {
                        region_bound: ty::ReStatic,
                        builtin_bounds: ty::BuiltinBounds::empty(),
                        projection_bounds: trait_ty.bounds.projection_bounds.fold_with(self),
                    }
                };

                self.tcx.mk_ty(ty::TyTrait(Box::new(trait_ty)))
            }
...
pub fn get_drop_glue_type<'tcx>(tcx: &TyCtxt<'tcx>,
                                t: Ty<'tcx>) -> Ty<'tcx> {
    let mut folder = GetDropGlueTypeFolder{ tcx: tcx };

    // Even if there is no dtor for t, there might be one deeper down and we
    // might need to pass in the vtable ptr.
    if !type_is_sized(tcx, t) {
        return tcx.erase_regions(&t).fold_with(&mut folder);
    }
    ...
    match t.sty {
        ty::TyBox(typ) if !type_needs_drop(&tcx, typ)
            ...
            if layout.size(&tcx.data_layout).bytes() == 0 {
                ...
                tcx.erase_regions(&t).fold_with(&mut folder)
            }
        }
        _ => tcx.erase_regions(&t).fold_with(&mut folder)
    }
}
