...
fn collect_items_rec<'a, 'tcx: 'a>(ccx: &CrateContext<'a, 'tcx>,
    ...
    record_inlining_canditates(ccx, starting_point, &neighbors[..], inlining_map);
...
fn record_inlining_canditates<'a, 'tcx>(ccx: &CrateContext<'a, 'tcx>,
                                        caller: TransItem<'tcx>,
                                        callees: &[TransItem<'tcx>],
                                        inlining_map: &mut InliningMap<'tcx>) {
    let is_inlining_candidate = |trans_item: &TransItem<'tcx>| {
        trans_item.is_from_extern_crate() || trans_item.requests_inline(ccx.tcx())
    };
