...
fn collect_roots<'a, 'tcx>(ccx: &CrateContext<'a, 'tcx>)
...

    {
        ...
        hir_visit::walk_crate(&mut visitor, ccx.tcx().map.krate());
    }
...
fn collect_items_rec<'a, 'tcx: 'a>(ccx: &'a CrateContext<'a, 'tcx>,
...
fn create_inlined_fn_codegen_item<'a, 'tcx>(ccx: &CrateContext<'a, 'tcx>,
    ...
    fn ignored(node: hir_map::Node) -> bool {
        match node {
            ...
            hir_map::NodeImplItem(&hir::ImplItem { node: hir::MethodImplItem(..), .. }) |
...
impl<'b, 'a, 'v> hir_visit::Visitor<'v> for RootCollector<'b, 'a, 'v> {
    ...
    fn visit_impl_item(&mut self, ii: &'v hir::ImplItem) {
        match ii.node {
            hir::MethodImplItem(hir::MethodSig { ref generics, .. }, _) => {
                let is_impl_generic = match self.enclosing_item {
                    Some(&hir::Item {
