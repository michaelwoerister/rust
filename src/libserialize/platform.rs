...
#[inline(always)]
fn write_raw<T>(cursor: &mut io::Cursor<Vec<u8>>, value_ptr: &T) -> EncodeResult {
    let slice = unsafe {
        ::std::slice::from_raw_parts(value_ptr as *const T as *const u8,
                                     ::std::mem::size_of::<T>())
    };
...
impl<'a> Decoder<'a> {
    fn read_raw<T>(&mut self) -> T {
        let mut result: T = unsafe {
            ::std::mem::uninitialized()
        };

        let source = (&self.data[self.position..]).as_ptr();

        unsafe {
            ::std::ptr::copy_nonoverlapping(source,
