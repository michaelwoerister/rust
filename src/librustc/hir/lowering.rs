...
const STABLE_COUNTER_ID_LOCKED: u32 = 0xFFFFFFFF;
...
pub struct LoweringContext<'a> {
    ...
    local_node_id_owner: Vec<(DefIndex, u32)>,
    ast_id_to_hir_id: Vec<hir::StableNodeId>,

    stable_node_id_counters: NodeMap<u32>,
}
...
pub fn lower_crate(sess: &Session,
    ...
    LoweringContext {
        ...
        local_node_id_owner: vec![(CRATE_DEF_INDEX, 1)],
        ast_id_to_hir_id: vec![],
        stable_node_id_counters: NodeMap(),
    }.lower_crate(krate)
}
...
impl<'a> LoweringContext<'a> {
    fn lower_crate(mut self, c: &Crate) -> hir::Crate {
        ...
        impl<'lcx, 'interner> Visitor<'lcx> for MiscCollector<'lcx, 'interner> {
            fn visit_item(&mut self, item: &'lcx Item) {
                self.lctx.allocate_hir_node_id_counter(item.id, item);
        ...
        impl<'lcx, 'interner> Visitor<'lcx> for ItemLowerer<'lcx, 'interner> {
            fn visit_item(&mut self, item: &'lcx Item) {
                ...
                self.lctx.with_stable_node_id_owner(item.id, |lctx| {
            ...
            fn visit_trait_item(&mut self, item: &'lcx TraitItem) {
                // // println!("lowering trait item {:?}", item);
                // let def_index = self.lctx.resolver.definitions().opt_def_index(item.id).unwrap();
                // self.lctx.local_node_id_owner.push((def_index, 0));
            ...
            fn visit_impl_item(&mut self, item: &'lcx ImplItem) {
                // println!("lowering impl item {:?}", item);
                // let def_index = self.lctx.resolver.definitions().opt_def_index(item.id).unwrap();
                // self.lctx.local_node_id_owner.push((def_index, 0));
    ...
    fn allocate_hir_node_id_counter<T: Debug>(&mut self,
                                              owner: NodeId,
                                              debug: &T) {
        if self.stable_node_id_counters.insert(owner, 0).is_some() {
            bug!("Tried to allocate stable_node_id_counter for {:?} twice",
                 debug);
        }
        // Always allocate the first stable id for the owner itself
        self.lower_node_id_with_owner(owner, owner);
    }

    fn lower_node_id_generic<F>(&mut self,
                                ast_node_id: NodeId,
                                alloc_stable_node_id: F)
                                -> NodeId
        where F: FnOnce(&mut Self) -> hir::StableNodeId
    {
        if ast_node_id == DUMMY_NODE_ID {
            return ast_node_id;
        }

        let ast_node_id = ast_node_id.as_usize();

        if ast_node_id >= self.ast_id_to_hir_id.len() {
            self.ast_id_to_hir_id.resize(ast_node_id + 1, hir::DUMMY_NODE_ID);
        }

        if self.ast_id_to_hir_id[ast_node_id] == hir::DUMMY_NODE_ID {
            // Generate a new stable ID
            self.ast_id_to_hir_id[ast_node_id] = alloc_stable_node_id(self);
        }

        NodeId::new(ast_node_id)
    }

    fn with_stable_node_id_owner<F>(&mut self, owner: NodeId, f: F)
        where F: FnOnce(&mut Self)
    {
        let counter = self.stable_node_id_counters
                          .insert(owner, STABLE_COUNTER_ID_LOCKED)
                          .unwrap();
        let def_index = self.resolver.definitions().opt_def_index(owner).unwrap();
        self.local_node_id_owner.push((def_index, counter));
        f(self);
        let (new_def_index, new_counter) = self.local_node_id_owner.pop().unwrap();

        debug_assert!(def_index == new_def_index);
        debug_assert!(new_counter >= counter);

        let prev = self.stable_node_id_counters.insert(owner, new_counter).unwrap();
        debug_assert!(prev == STABLE_COUNTER_ID_LOCKED);
    }
    ...
    fn lower_node_id_with_owner(&mut self,
        ...
        self.lower_node_id_generic(ast_node_id, |this| {
            let local_id_counter = this.stable_node_id_counters
                                       .get_mut(&owner)
                                       .unwrap();
            ...
            debug_assert!(local_id != STABLE_COUNTER_ID_LOCKED);
    ...
    fn lower_node_id(&mut self, ast_node_id: NodeId) -> NodeId {
        self.lower_node_id_generic(ast_node_id, |this| {
            let &mut (def_index, ref mut local_id_counter) = this.local_node_id_owner
                                                                 .last_mut()
                                                                 .unwrap();
            let local_id = *local_id_counter;
            *local_id_counter += 1;
        ...
        label.map(|sp_ident| {
            hir::Label {
                ...
                loop_id: match self.expect_full_def(id) {
                    Def::Label(loop_id) => self.lower_node_id(loop_id),
                    _ => DUMMY_NODE_ID
                }
            }
        })
        ...
        match e.node {
            ...
            ExprKind::Box(ref inner) => {
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprBox(P(self.lower_expr(inner)))
                }
            }
            ...
            ExprKind::Array(ref exprs) => {
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprArray(exprs.iter().map(|x| self.lower_expr(x)).collect())
                }
            }
            ExprKind::Repeat(ref expr, ref count) => {
                ...
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprRepeat(expr, self.record_body(count, None))
                }
            }
            ExprKind::Tup(ref elts) => {
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprTup(elts.iter().map(|x| self.lower_expr(x)).collect())
                }
            }
            ExprKind::Call(ref f, ref args) => {
                ...
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprCall(f, args.iter().map(|x| self.lower_expr(x)).collect())
                }
            }
            ExprKind::MethodCall(i, ref tps, ref args) => {
                ...
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprMethodCall(respan(i.span, i.node.name), tps, args)
                }
            }
            ExprKind::Binary(binop, ref lhs, ref rhs) => {
                ...
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprBinary(binop, lhs, rhs)
                }
            }
            ...
            ExprKind::Cast(ref expr, ref ty) => {
                ...
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprCast(expr, self.lower_ty(ty))
                }
            }
            ExprKind::Type(ref expr, ref ty) => {
                ...
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprType(expr, self.lower_ty(ty))
                }
            }
            ...
            ExprKind::If(ref cond, ref blk, ref else_opt) => {
                ...
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprIf(P(self.lower_expr(cond)), self.lower_block(blk), else_opt)
                }
            }
            ExprKind::While(ref cond, ref body, opt_ident) => {
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprWhile(P(self.lower_expr(cond)),
            ...
            ExprKind::Loop(ref body, opt_ident) => {
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprLoop(self.lower_block(body),
                                        self.lower_opt_sp_ident(opt_ident),
                                        hir::LoopSource::Loop)
                }
            }
            ExprKind::Match(ref expr, ref arms) => {
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprMatch(P(self.lower_expr(expr)),
            ...
            ExprKind::Closure(capture_clause, ref decl, ref body, fn_decl_span) => {
                self.with_parent_def(e.id, |this| {
                    ...
                    hir::Expr {
                        id: this.lower_node_id(e.id),
                        span: e.span,
                        attrs: e.attrs.clone(),
                        node: hir::ExprClosure(this.lower_capture_clause(capture_clause),
            ...
            ExprKind::Block(ref blk) => {
                hir::Expr {
                    id: self.lower_node_id(e.id),
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprBlock(self.lower_block(blk))
                }
            }
            ExprKind::Assign(ref el, ref er) => {
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprAssign(P(self.lower_expr(el)), P(self.lower_expr(er)))
                }
            }
            ExprKind::AssignOp(op, ref el, ref er) => {
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprAssignOp(self.lower_binop(op),
            ...
            ExprKind::Field(ref el, ident) => {
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprField(P(self.lower_expr(el)), respan(ident.span, ident.node.name))
                }
            }
            ExprKind::TupField(ref el, ident) => {
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprTupField(P(self.lower_expr(el)), ident)
                }
            }
            ExprKind::Index(ref el, ref er) => {
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprIndex(P(self.lower_expr(el)), P(self.lower_expr(er)))
                }
            }
            ...
            ExprKind::Path(ref qself, ref path) => {
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprPath(self.lower_qpath(e.id, qself, path, ParamMode::Optional))
                }
            }
            ExprKind::Break(opt_ident, ref opt_expr) => {
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprBreak(self.lower_label(e.id, opt_ident),
            ...
            ExprKind::Continue(opt_ident) => {
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprAgain(self.lower_label(e.id, opt_ident))
                }
            }
            ExprKind::Ret(ref re) => {
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprRet(re.as_ref().map(|x| P(self.lower_expr(x))))
                }
            }
            ExprKind::InlineAsm(ref asm) => {
                ...
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprInlineAsm(P(hir_asm), outputs, inputs)
                }
            }
            ExprKind::Struct(ref path, ref fields, ref maybe_expr) => {
                hir::Expr {
                    ...
                    span: e.span,
                    attrs: e.attrs.clone(),
                    node: hir::ExprStruct(self.lower_qpath(e.id, &None, path, ParamMode::Optional),
            ...
            ExprKind::IfLet(ref pat, ref sub_expr, ref body, ref else_opt) => {
                ...
                let mut else_arms = vec![];

                // if let Some(else_expr) = else_opt.as_ref() {
                //     let mut current = else_expr;
                //     let mut else_exprs = vec![current];

                //     // while let ExprKind::If(_, _, ref else_expr_opt) = current.node {
                //     //     if let &Some(ref else_expr) = else_expr_opt {
                ...
                //     // }
                //     loop {
                //         if let ExprKind::If(_, _, ref else_expr_opt) = current.node {
                //             if let Some(ref else_expr) = *else_expr_opt {
                ...
                //         let (guard, body) = if let ExprKind::If(ref cond, ref then, _) = else_expr.node {
                //             (Some(cond), self.expr_block(self.lower_block(then), ThinVec::new()))
                //         } else {
                //             // This is the last one, just push it without guard
                //             (None, self.lower_expr(else_expr))
                //         };

                //         else_arms.push(hir::Arm {
                //             attrs: hir_vec![],
...

                {
                    ...
                    for else_expr in else_exprs {
                        if let Some(else_expr) = else_expr {
                            ...
                            else_arms.push(self.arm(hir_vec![pat_under], unit));
                        }
                    }
                }
                ...
                //     let pat_under = self.pat_wild(e.span);
                //     // let else_expr =
                //     //     else_opt.map(|e| P(self.lower_expr(e)))
                //     //             .unwrap_or_else(|| self.expr_tuple(e.span, hir_vec![]));
                //     let else_expr = if let &Some(ref else_expr) = else_opt {
    ...
    fn lower_stmt(&mut self, s: &Stmt) -> SmallVector<hir::Stmt> {
        ...
        SmallVector::one(match s.node {
            ...
            StmtKind::Item(ref it) => {
                ...
                let mut id = Some(self.lower_node_id(s.id));
            ...
            StmtKind::Expr(ref e) => {
                Spanned {
                    node: hir::StmtExpr(P(self.lower_expr(e)), self.lower_node_id(s.id)),
                    span: s.span,
                }
            }
            StmtKind::Semi(ref e) => {
                Spanned {
                    node: hir::StmtSemi(P(self.lower_expr(e)), self.lower_node_id(s.id)),
                    span: s.span,
                }
            }
    ...
    // If an explicit_owner is given, this method allocates the StableNodeId in
    // the address space of that item instead of the item currently being
    // lowered. This can happen during lower_impl_item_ref() where we need to
    // the Visibility value although we haven't lowered the owning ImplItem in
    // question yet.
