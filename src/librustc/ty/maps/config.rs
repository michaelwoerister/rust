...
pub(super) trait QueryDescription<'tcx>: QueryConfig {
    const ON_DISK_CACHE: bool = false;

    fn describe(tcx: TyCtxt, key: Self::Key) -> String;
    fn load_from_cache<'a>(_: TyCtxt<'a, 'tcx, 'tcx>,
                                 _: SerializedDepNodeIndex)
                                 -> Self::Value {
        panic!()
    }

    fn should_load_from_cache(_: Self::Key) -> bool {
        false
    }
}
...
impl<'tcx> QueryDescription<'tcx> for queries::optimized_mir<'tcx> {
    const ON_DISK_CACHE: bool = true;

    fn load_from_cache<'a>(tcx: TyCtxt<'a, 'tcx, 'tcx>,
    ...
    fn should_load_from_cache(key: DefId) -> bool {
        key.is_local()
    }
}
