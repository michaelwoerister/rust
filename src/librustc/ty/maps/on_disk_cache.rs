...
const QUERY_RESULT_INDEX_TAG: u64 = 0x1234_5678_FFFF_0000;
...
impl<'sess> OnDiskCache<'sess> {
    ...
    pub fn new(sess: &'sess Session, data: Vec<u8>, start_pos: usize) -> OnDiskCache<'sess> {
        ...
        let (header, prev_diagnostics, query_result_index) = {
            use ty::codec::TyDecoder;

            let mut decoder = opaque::Decoder::new(&data[..], start_pos);
            let header: Header = decode_tagged(&mut decoder, HEADER_TAG).unwrap();
            ...
            (header.clone(), prev_diagnostics, query_result_index)
        };
        ...
        where E: ty_codec::TyEncoder
    {
        ...
        for entry in tcx.maps.optimized_mir.borrow().map.values() {
            let index = SerializedDepNodeIndex::new(entry.index.index());
            query_result_index.push((index, encoder.position()));
            encoder.encode_tagged(index, &entry.value)?;
        }

        for entry in tcx.maps.typeck_tables_of.borrow().map.values() {
            let index = SerializedDepNodeIndex::new(entry.index.index());
            query_result_index.push((index, encoder.position()));
            encoder.encode_tagged(index, &entry.value)?;
        }
    ...
    pub fn load_query_result<'a, 'tcx, T: Decodable>(&self,
