...
void CompileUnit::constructTypeDIE(DIE &Buffer, DICompositeType CTy) {
  ...
  case dwarf::DW_TAG_class_type: {
    ...
    for (unsigned i = 0; i < N; ++i) {
      ...
      } else if (Element.isVariantPart()) {
        ElemDie = createVariantPartDIE(DIVariantPart(Element));
      } else
        continue;
      Buffer.addChild(ElemDie);
    }
...
/// createStaticMemberDIE - Create new DIE for C++ static member.
DIE *CompileUnit::createVariantPartDIE(const DIVariantPart VP) {
  if (!VP.Verify())
    return NULL;

  DIE *VariantPartDIE = new DIE(VP.getTag());

  // Handle discriminant
  DIDerivedType Discriminant = VP.getDiscriminant();
  assert(Discriminant.Verify() &&
    "CompileUnit::createVariantPartDIE: Invalid discriminant reference");

  DIEEntry *DiscriminantRef = getDIEEntry(Discriminant);
  if (!DiscriminantRef) {

    DIE *DiscriminantDIE = getDIE(Discriminant);
    assert(DiscriminantDIE &&
      "CompileUnit::createVariantPartDIE: Discriminant DIE must be created before variant part DIE");

    DiscriminantRef = createDIEEntry(DiscriminantDIE);
    insertDIEEntry(Discriminant, DiscriminantRef);
  }

  VariantPartDIE->addValue(dwarf::DW_AT_discr, dwarf::DW_FORM_ref4, DiscriminantRef);

  // Handle Variants
  DIArray Variants = VP.getVariants();

  for (unsigned i = 0; i < Variants.getNumElements(); ++i) {
    DIDescriptor Element = Variants.getElement(i);

    if(!Element.isVariant())
      continue;

    DIVariant Variant(Element);
    DIE *VariantDIE = new DIE(Variant.getTag());
    addSInt(VariantDIE, dwarf::DW_AT_discr_value, 0, Variant.getDiscriminant());

    DIArray VariantMembers = Variant.getMembers();

    for (unsigned j = 0; j < VariantMembers.getNumElements(); ++j) {
      DIDerivedType Member(VariantMembers.getElement(j));
      if (!Member.Verify())
        continue;

      VariantDIE->addChild(createMemberDIE(Member));
    }

    VariantPartDIE->addChild(VariantDIE);
  }

  insertDIE(VP, VariantPartDIE);

  return VariantPartDIE;
}
