...
bool DIDescriptor::Verify() const {
  return DbgNode &&
         (DIDerivedType(DbgNode).Verify() ||
          ...
          DIVariantPart(DbgNode).Verify() || DIVariant(DbgNode).Verify());
}
...
/// isVariantPart - Return true if the specified tag is DW_TAG_variant_part.
bool DIDescriptor::isVariantPart() const {
  return DbgNode && getTag() == dwarf::DW_TAG_variant_part;
}

/// isVariant - Return true if the specified tag is DW_TAG_variant.
bool DIDescriptor::isVariant() const {
  return DbgNode && getTag() == dwarf::DW_TAG_variant;
}
...
/// \brief Verify that the variant part descriptor is well formed.
bool DIVariantPart::Verify() const {
  return isVariantPart() && DbgNode->getNumOperands() == 3;
}

/// \brief Verify that the variant descriptor is well formed.
bool DIVariant::Verify() const {
  return isVariant() && DbgNode->getNumOperands() == 3;
}
...
void DIDescriptor::print(raw_ostream &OS) const {
  ...
  } else if (this->isVariantPart()) {
    DIVariantPart(DbgNode).printInternal(OS);
  } else if (this->isVariant()) {
    DIVariant(DbgNode).printInternal(OS);
...
void DIVariantPart::printInternal(raw_ostream &OS) const {
  OS << "[VariantPart]";
}

void DIVariant::printInternal(raw_ostream &OS) const {
  OS << " [discr " << getDiscriminant() << ']';
}
