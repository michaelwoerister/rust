...
/// createVariantPart - Create debugging information entry for the variant part of a struct.
DIVariantPart DIBuilder::createVariantPart(DIDerivedType DiscrField,
                                           DIArray Variants) {
  Value *Elts[] = {
    GetTagConstant(VMContext, dwarf::DW_TAG_variant_part),
    DiscrField,
    Variants
  };
  return DIVariantPart(MDNode::get(VMContext, Elts));
}

/// createVariant - Create debugging information entry for one variant.
DIVariant DIBuilder::createVariant(uint64_t DiscrValue,
                                   DIArray Members) {
  Value *Elts[] = {
    GetTagConstant(VMContext, dwarf::DW_TAG_variant),
    ConstantInt::get(Type::getInt64Ty(VMContext), DiscrValue),
    Members
  };
  return DIVariant(MDNode::get(VMContext, Elts));
}
