...
namespace llvm {
  ...
  class DIBuilder {
    ...
    /// @param DiscrField   A reference to the discriminant field in the containing type.
    /// @param Variants     Variant elements.
    DIVariantPart createVariantPart(DIDerivedType DiscrField, DIArray Variants);

    /// createVariant - Create debugging information entry for one variant.
    /// @param discriminant   The discriminant value of the variant.
    /// @param Members        The member type descriptors of this variant.
    DIVariant createVariant(uint64_t DiscrValue, DIArray Members);
