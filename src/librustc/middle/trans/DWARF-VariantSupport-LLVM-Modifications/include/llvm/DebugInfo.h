...
namespace llvm {
  ...
  /// DIVariantPart
  class DIVariantPart : public DIDescriptor {
    friend class DIDescriptor;
    void printInternal(raw_ostream &OS) const;
  public:
    explicit DIVariantPart(const MDNode *N = 0) : DIDescriptor(N) {}

    DIDerivedType getDiscriminant() const { return getFieldAs<DIDerivedType>(1); }
    DIArray getVariants() const { return getFieldAs<DIArray>(2); }

    bool Verify() const;
  };

  /// DIVariant
  class DIVariant : public DIDescriptor {
    friend class DIDescriptor;
    void printInternal(raw_ostream &OS) const;
  public:
    explicit DIVariant(const MDNode *N = 0) : DIDescriptor(N) {}

    uint64_t getDiscriminant() const { return getUInt64Field(1); }
    DIArray getMembers() const { return getFieldAs<DIArray>(2); }
