    ...
    fn hash_stable<W: StableHasherResult>(&self,
        ...
        match *self {
            ...
            ConstVal::Function(def_id, substs) => {
                ...
                // let node_id_hashing_mode = if def_id.is_local() {
...
impl<'a, 'gcx, 'tcx> HashStable<StableHashingContext<'a, 'gcx, 'tcx>>
for ty::TypeParameterDef {
    fn hash_stable<W: StableHasherResult>(&self,
                                          hcx: &mut StableHashingContext<'a, 'gcx, 'tcx>,
                                          hasher: &mut StableHasher<W>) {
        let ty::TypeParameterDef {
            name,
            def_id,
            index,
            has_default,
        ...
        // TODO: Don't run into NodeIds from other crates...
        let node_id_hashing_mode = if def_id.is_local() {
            NodeIdHashingMode::HashDefPath
        } else {
            NodeIdHashingMode::Ignore
        ...
        hcx.with_node_id_hashing_mode(NodeIdHashingMode::HashDefPath, |hcx| {
            match *self {
                ...
                CodeExtent::ParameterScope(body_id) => {
                    body_id.node_id.hash_stable(hcx, hasher);
                }
...
impl<'a, 'gcx, 'tcx, T> HashStable<StableHashingContext<'a, 'gcx, 'tcx>> for ty::steal::Steal<T>
    where T: HashStable<StableHashingContext<'a, 'gcx, 'tcx>>
{
    fn hash_stable<W: StableHasherResult>(&self,
                                          hcx: &mut StableHashingContext<'a, 'gcx, 'tcx>,
